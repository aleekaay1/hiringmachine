-- Support ticketing + ops health snapshots
...
-- Ops snapshots: no direct client access (edge function uses service role)

comment on table public.support_tickets is 'Staff support tickets submitted from /support';
comment on table public.support_ticket_events is 'Timeline entries visible to submitter when visible_to_user=true';
comment on table public.ops_health_snapshots is 'Manual ops health captures (ali-only via edge function)';
