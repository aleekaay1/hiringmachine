-- Per-session registry: Calendly registrations + Zoom attendance (updated on each Refresh sync).
...
comment on table public.live_session_occurrences is 'Wednesday Live Online Career Session — one row per date; synced from Calendly + Zoom.';
comment on table public.live_session_registrants is 'Registrants per session (Calendly) with optional Zoom attendance match.';
