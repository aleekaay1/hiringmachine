...
function pickPreferredParticipant<T extends ZoomParticipantLike>(a: T, b: T): T {
  const emailA = effectiveZoomEmail(a);
  const emailB = effectiveZoomEmail(b);
  if (emailA && !emailB) return a;
  if (emailB && !emailA) return b;
  const nameA = cleanZoomDisplayName(a.name ?? '');
  const nameB = cleanZoomDisplayName(b.name ?? '');
  if (nameB.length > nameA.length) return b;
  if (nameA.length > nameB.length) return a;
  const joinA = Date.parse(String(a.join_time || ''));
  const joinB = Date.parse(String(b.join_time || ''));
  if (Number.isFinite(joinB) && (!Number.isFinite(joinA) || joinB < joinA)) return b;
  return a;
}

/** Merge reconnect rows and short Zoom display names (e.g. "Nikita" + "Nikita Mehta"). */
export function dedupeZoomParticipants<T extends ZoomParticipantLike>(participants: T[]): T[] {
  ...
  for (const participant of participants) {
    ...
    for (let i = 0; i < merged.length; i += 1) {
      ...
      const emailA = effectiveZoomEmail(existing);
      const emailB = effectiveZoomEmail(participant);
      const sameEmail = Boolean(emailA && emailB && emailA === emailB);
      const sameName = samePersonByName(String(existing.name || ''), String(participant.name || ''));
      if (sameEmail || sameName) {
        merged[i] = pickPreferredParticipant(existing, participant);
        found = true;
        break;
      }
    }
    if (!found) merged.push(participant);
  }

  const byKey = new Map<string, T>();
  for (const participant of merged) {
    ...
    byKey.set(key, pickPreferredParticipant(existing, participant));
  }
  return [...byKey.values()];
}

function isUnmatchedZoomParticipant<T extends CalInviteeLike>(
  participant: ZoomParticipantLike,
  matchedInvitees: Array<MatchedInvitee<T>>,
  usedKeys: Set<string>,
  ...
  for (const inv of matchedInvitees) {
    if (!inv.attended_zoom) continue;
    if (matchParticipantToInvitee(inv, participant)) return false;
    if (samePersonByName(String(inv.name || ''), String(participant.name || ''))) return false;
    const calLocal = emailLocalPart(inv.email);
    const zoomCompact = compactAlpha(String(participant.name || ''));
    if (calLocal.length >= 5 && zoomCompact.length >= 4 && calLocal.includes(zoomCompact)) return false;
  }

  for (const used of dedupedParticipants) {
    const usedKey = participantIdentityKey(used);
    if (!usedKey || !usedKeys.has(usedKey)) continue;
    if (used === participant) continue;
    if (samePersonByName(String(used.name || ''), String(participant.name || ''))) return false;
    const emailA = effectiveZoomEmail(used);
    const emailB = effectiveZoomEmail(participant);
    if (emailA && emailB && emailA === emailB) return false;
  }

  return Boolean(participant.name || participant.user_email || key);
}
...
function matchParticipantToInvitee<T extends CalInviteeLike>(
  ...
  if (calLocal.length >= 5) {
    ...
    if (zoomNameLocal.length >= 4 && calLocal.includes(zoomNameLocal)) return 'hybrid';
...
export function matchInviteesToParticipants<T extends CalInviteeLike>(
  ...
  const walkinParticipants = dedupedParticipants.filter((participant) =>
    isUnmatchedZoomParticipant(participant, invitees, usedKeys, dedupedParticipants),
  );
...
/** Zoom joiners with no Calendly registration (after dedupe + fuzzy match). */
export function listUnmatchedZoomParticipants<T extends CalInviteeLike>(
  rawInvitees: T[],
  participants: ZoomParticipantLike[],
): ZoomParticipantLike[] {
  return matchInviteesToParticipants(rawInvitees, participants).walkinParticipants;
}
